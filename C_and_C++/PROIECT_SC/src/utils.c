#include "utils.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#ifdef _WIN32
#include <windows.h>

typedef BOOLEAN (WINAPI *RtlGenRandom_t)(PVOID, ULONG);

void rand_bytes(uint8_t *buf, size_t len) {
    static HMODULE hAdvapi = NULL;
    static RtlGenRandom_t pRtlGenRandom = NULL;

    if (!hAdvapi) {
        hAdvapi = LoadLibraryA("advapi32.dll");
        if (!hAdvapi) die("Cannot load advapi32.dll");
        
        pRtlGenRandom = (RtlGenRandom_t)(void *)GetProcAddress(hAdvapi, "SystemFunction036");
        if (!pRtlGenRandom) die("Cannot find SystemFunction036 in advapi32.dll");
    }

    if (!pRtlGenRandom(buf, (ULONG)len)) {
        die("RtlGenRandom failed");
    }
}
#else
void rand_bytes(uint8_t *buf, size_t len) {
    FILE *f = fopen("/dev/urandom", "rb");
    if (!f) die("Cannot open /dev/urandom");
    if (fread(buf, 1, len, f) != len) die("rand_bytes: short read");
    fclose(f);
}
#endif

void sha256(const uint8_t *msg, size_t len, uint8_t out[32]) {
    static const uint32_t K[64] = {
        0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,
        0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
        0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,
        0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
        0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,
        0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
        0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,
        0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
        0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,
        0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
        0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,
        0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
        0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,
        0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
        0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,
        0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
    };
    uint32_t h[8] = {
        0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
        0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19
    };

#define ROTR32(x,n) (((x)>>(n))|((x)<<(32-(n))))
#define CH(e,f,g)  (((e)&(f))^(~(e)&(g)))
#define MAJ(a,b,c) (((a)&(b))^((a)&(c))^((b)&(c)))
#define EP0(a) (ROTR32(a,2)^ROTR32(a,13)^ROTR32(a,22))
#define EP1(e) (ROTR32(e,6)^ROTR32(e,11)^ROTR32(e,25))
#define SIG0(x)(ROTR32(x,7)^ROTR32(x,18)^((x)>>3))
#define SIG1(x)(ROTR32(x,17)^ROTR32(x,19)^((x)>>10))

    size_t total = len;
    size_t padded_len = ((len + 8) / 64 + 1) * 64;
    uint8_t *buf = calloc(padded_len, 1);
    memcpy(buf, msg, len);
    buf[len] = 0x80;
    uint64_t bit_len = (uint64_t)total * 8;
    for (int i = 0; i < 8; i++)
        buf[padded_len - 1 - i] = (bit_len >> (i * 8)) & 0xFF;

    for (size_t chunk = 0; chunk < padded_len; chunk += 64) {
        uint32_t w[64];
        for (int i = 0; i < 16; i++)
            w[i] = ((uint32_t)buf[chunk+i*4]<<24)|((uint32_t)buf[chunk+i*4+1]<<16)|
                   ((uint32_t)buf[chunk+i*4+2]<<8)|(uint32_t)buf[chunk+i*4+3];
        for (int i = 16; i < 64; i++)
            w[i] = SIG1(w[i-2]) + w[i-7] + SIG0(w[i-15]) + w[i-16];

        uint32_t a=h[0],b=h[1],c=h[2],d=h[3],
                 e=h[4],f=h[5],g=h[6],hh=h[7];
        for (int i = 0; i < 64; i++) {
            uint32_t t1 = hh + EP1(e) + CH(e,f,g) + K[i] + w[i];
            uint32_t t2 = EP0(a) + MAJ(a,b,c);
            hh=g; g=f; f=e; e=d+t1;
            d=c;  c=b; b=a; a=t1+t2;
        }
        h[0]+=a; h[1]+=b; h[2]+=c; h[3]+=d;
        h[4]+=e; h[5]+=f; h[6]+=g; h[7]+=hh;
    }
    free(buf);
    for (int i = 0; i < 8; i++) {
        out[i*4]   = (h[i]>>24)&0xFF; out[i*4+1] = (h[i]>>16)&0xFF;
        out[i*4+2] = (h[i]>> 8)&0xFF; out[i*4+3] = (h[i]    )&0xFF;
    }
}

void xor_bytes(uint8_t *dst, const uint8_t *a, const uint8_t *b, size_t len) {
    for (size_t i = 0; i < len; i++) dst[i] = a[i] ^ b[i];
}

int read_key_file(const char *path, uint8_t *buf, size_t expected_len) {
    FILE *f = fopen(path, "rb");
    if (!f) { perror("fopen key"); return -1; }
    size_t n = fread(buf, 1, expected_len, f);
    fclose(f);
    if (n != expected_len) {
        fprintf(stderr, "Key file must be exactly %zu bytes\n", expected_len);
        return -1;
    }
    return 0;
}

int write_file(const char *path, const uint8_t *buf, size_t len) {
    FILE *f = fopen(path, "wb");
    if (!f) { perror("fopen write"); return -1; }
    fwrite(buf, 1, len, f);
    fclose(f);
    return 0;
}

void print_hex(const char *label, const uint8_t *buf, size_t len) {
    printf("%s: ", label);
    for (size_t i = 0; i < len; i++) printf("%02x", buf[i]);
    printf("\n");
}

void die(const char *msg) {
    fprintf(stderr, "Error: %s\n", msg);
    exit(1);
}